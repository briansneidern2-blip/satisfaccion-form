...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function scrollToFirstInvalid() {
    const activePage = document.querySelector('.form-step-page.active');
    if (!activePage) return;
    const firstInvalid = activePage.querySelector('.invalid');
    if (firstInvalid) {
  ...
  function validateStep(step) {
    ...
    if (step === 2) {
      ...
      const hiddenAspectos = document.getElementById('aspectosSeleccionados');
      const tagsSection = document.getElementById('tagsAspectosSection');
      if (!hiddenAspectos || !hiddenAspectos.value || hiddenAspectos.value.trim() === '') {
    ...
    // Paso 3: Opinión final obligatoria y consentimiento de datos
    if (step === 3) {
      const elComentarios = document.getElementById('comentariosSugerencias');
      const parentComentarios = elComentarios ? elComentarios.closest('.input-group') : null;

      if (!elComentarios || !elComentarios.value || !elComentarios.value.trim()) {
        if (parentComentarios) parentComentarios.classList.add('invalid');
        isValid = false;
      } else {
        if (parentComentarios) parentComentarios.classList.remove('invalid');
      }

      const aceptaPoliticas = document.getElementById('aceptaPoliticas');
      const policyContainer = aceptaPoliticas ? aceptaPoliticas.closest('.policy-checkbox-container') : null;
      if (!aceptaPoliticas || !aceptaPoliticas.checked) {
        if (policyContainer) {
          policyContainer.classList.remove('shake-checkbox');
          void policyContainer.offsetWidth; // Disparar reinicio de animación CSS
          policyContainer.classList.add('invalid', 'shake-checkbox');
        }
        isValid = false;
      } else {
        if (policyContainer) {
          policyContainer.classList.remove('invalid', 'shake-checkbox');
        }
      }
    }
  ...
  stepItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (targetStep < currentStep) {
        ...
        // Validar secuencialmente todos los pasos intermedios antes de permitir avanzar
        let canAdvance = true;
        for (let s = currentStep; s < targetStep; s++) {
          if (!validateStep(s)) {
            canAdvance = false;
            updateStepper(s, false); // Cambiar al paso faltante sin scroll al top del hero
            setTimeout(scrollToFirstInvalid, 50); // Desplazar suavemente a la primera opción no seleccionada
            break;
          }
        }
        if (canAdvance) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Recopilar datos para guardar en Google Sheets tal como los escribe el usuario
    const payload = {
      ...
      comentariosSugerencias: (document.getElementById('comentariosSugerencias').value || '').trim()
    };
